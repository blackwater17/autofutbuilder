...
MongoClient.connect(connectionURL, {useNewUrlParser: true}, (error, client) => {
    ...
    db.collection("complete_players").find().toArray().then((all_players_list) => {
        ...
        while(i>=0) {
            ...
            let j=i
            while(j>=0) {
                if(isSamePlayer(current_player, all_players_list[j]) && hasWrongBase(current_player, all_players_list[j])) {
                    update_someone(all_players_list[j]._id , current_player.base_card_id)
                }
                j-=1
            }
